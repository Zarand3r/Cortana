...
def test_prune_by_size(tmp_path):
    ...
    before = counts(mem)["context"]
    mem.prune()
    after = counts(mem)["context"]
...
def test_orphan_summaries_pruned(tmp_path):
    ...
    assert counts(mem)["summaries"] == 2
    mem.prune()
    assert counts(mem)["summaries"] == 1              # orphan removed (P4)
    mem.close()
...
def test_prune_keeps_fts_in_sync(tmp_path):
    ...
    mem.prune()
    assert counts(mem)["context_fts"] == counts(mem)["context"]    # P2
    mem.close()
